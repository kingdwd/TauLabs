...
/*
 * SPI3 Interface
 * Used for the W25X40 External SPI DATA FLASH
 */
void PIOS_SPI_extdf_irq_handler(void);
void DMA1_Stream0_IRQHandler(void) __attribute__((alias("PIOS_SPI_extdf_irq_handler")));
void DMA1_Stream5_IRQHandler(void) __attribute__((alias("PIOS_SPI_extdf_irq_handler")));
...
uint32_t pios_spi_extdf_id;
void PIOS_SPI_extdf_irq_handler(void)
{
	/* Call into the generic code to handle the IRQ for this specific device */
	PIOS_SPI_IRQ_Handler(pios_spi_extdf_id);
}
...
static const struct pios_usart_cfg pios_usart6_dsm_cfg = {
.regs = USART6,
.remap = GPIO_AF_USART6,
.init = {
...
static const struct pios_sbus_cfg pios_usart6_sbus_aux_cfg = {
	/* No inverter configuration, usart6 in uBrain is hardware inverted */
};
