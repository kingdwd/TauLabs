 ...
 */
#include "pios.h"
#include "pios_flash_at45_priv.h"



// AT45DB161D Commands (from Datasheet)
#define AT45_PAGE_TO_BUFFER_1        0x53
#define AT45_PAGE_TO_BUFFER_2        0x55
#define AT45_READ_STATUS             0xD7
#define AT45_DEVICE_ID               0x9F
#define AT45_PAGE_WRITE              0x02
...
#define AT45_MM_PAGE_PROGRAM         0x82
#define AT45_BUFFER1_MM_PROGRAM      0x83
#define AT45_BUFFER2_MM_PROGRAM      0x86
...
#define AT45_STATUS_BUSY             0x80

enum pios_AT45_dev_magic {
	PIOS_AT45_DEV_MAGIC = 0xcb55aa55,
};

//! Device handle structure
struct AT45_flash_dev {
	uint32_t spi_id;
	uint32_t slave_num;
	bool claimed;
	...
	const struct pios_flash_at45_cfg * cfg;
#if defined(FLASH_FREERTOS)
	xSemaphoreHandle transaction_lock;
#endif
	enum pios_AT45_dev_magic magic;
};

//! Private functions
static int32_t PIOS_Flash_AT45_Validate(struct AT45_flash_dev * flash_dev);
static struct AT45_flash_dev * PIOS_Flash_AT45_alloc(void);

static int32_t PIOS_Flash_AT45_ReadID(struct AT45_flash_dev * flash_dev);
static int32_t PIOS_Flash_AT45_ReadStatus(struct AT45_flash_dev * flash_dev);
static int32_t PIOS_Flash_AT45_ClaimBus(struct AT45_flash_dev * flash_dev);
static int32_t PIOS_Flash_AT45_ReleaseBus(struct AT45_flash_dev * flash_dev);
static int32_t PIOS_Flash_AT45_Busy(struct AT45_flash_dev * flash_dev);
...
static struct AT45_flash_dev * PIOS_Flash_AT45_alloc(void)
{
	struct AT45_flash_dev * flash_dev;

	flash_dev = (struct AT45_flash_dev *)pvPortMalloc(sizeof(*flash_dev));
	if (!flash_dev) return (NULL);

	flash_dev->claimed = false;
	flash_dev->magic = PIOS_AT45_DEV_MAGIC;
#if defined(FLASH_FREERTOS)
...
static int32_t PIOS_Flash_AT45_Validate(struct AT45_flash_dev * flash_dev) {
	if (flash_dev == NULL)
		return -1;
	if (flash_dev->magic != PIOS_AT45_DEV_MAGIC)
		return -2;
...
{
	struct AT45_flash_dev * flash_dev = PIOS_Flash_AT45_alloc();
	if (flash_dev == NULL)
		return -1;
	...
	(void) PIOS_Flash_AT45_ReadID(flash_dev);
...
static int32_t PIOS_Flash_AT45_Busy(struct AT45_flash_dev * flash_dev)
{
	int32_t status = PIOS_Flash_AT45_ReadStatus(flash_dev);
	if (status < 0)
		return -1;
	return status & AT45_STATUS_BUSY;
}
...
static int32_t PIOS_Flash_AT45_ReadStatus(struct AT45_flash_dev * flash_dev)
{
	uint8_t out[] = {AT45_READ_STATUS, 0x00};
	uint8_t in[] = {0,0};
 ...
 */
static int32_t PIOS_Flash_AT45_StartTransaction(uintptr_t flash_id)
{
	struct AT45_flash_dev * flash_dev = (struct AT45_flash_dev *)flash_id;

	if (PIOS_Flash_AT45_Validate(flash_dev) != 0)
		return -1;
 ...
 * @return 0 for success, -1 for timeout
 */
static int32_t PIOS_Flash_AT45_EndTransaction(uintptr_t flash_id)
{
	struct AT45_flash_dev * flash_dev = (struct AT45_flash_dev *)flash_id;
 ...
 * @retval
 */
static int32_t PIOS_Flash_AT45_EraseSector(uintptr_t flash_id, uint32_t addr)
{
	struct AT45_flash_dev * flash_dev = (struct AT45_flash_dev *)flash_id;
	...
	uint8_t out[] = {AT45_PAGE_ERASE, (uint8_t)(addr >> 6) , (uint8_t)(addr << 2) , 0x00 };
	...
	while(!PIOS_Flash_AT45_Busy(flash_dev)) {
#if defined(FLASH_FREERTOS)
		vTaskDelay(1);
#endif
	}
 ...
 */
static int32_t PIOS_Flash_AT45_EraseChip(uintptr_t flash_id)
{
	struct AT45_flash_dev * flash_dev = (struct AT45_flash_dev *)flash_id;
 ...
 */
int32_t PIOS_Flash_AT45_WriteData(uintptr_t flash_id, uint16_t addr, uint8_t * data, uint16_t len)
{
	struct AT45_flash_dev * flash_dev = (struct AT45_flash_dev *)flash_id;
	uint32_t offset = 0;
 ...
 * @retval -1 Unable to claim SPI bus
 */
static int32_t PIOS_Flash_AT45_ReadData(uintptr_t flash_id, uint32_t addr, uint16_t offset, uint8_t * data, uint16_t len)
{
	struct AT45_flash_dev * flash_dev = (struct AT45_flash_dev *)flash_id;
	...
	if (PIOS_Flash_AT45_ClaimBus(flash_dev) == -1)
		return -1;

	uint8_t out[] = {AT45_PAGE_READ, (uint8_t)(addr >> 6) , (uint8_t)((addr << 2) | (offset >> 8)), (uint8_t)(offset), 0x00, 0x00, 0x00, 0x00};
	if(PIOS_SPI_TransferBlock(flash_dev->spi_id,out,NULL,sizeof(out),NULL) < 0) {
		PIOS_Flash_AT45_ReleaseBus(flash_dev);
		return -2;
	}
 ...
 * @retval -1 Unable to claim SPI bus
 */
int32_t PIOS_Flash_AT45_ReadPage(uintptr_t flash_id, uint32_t addr, uint16_t offset, uint8_t * data, uint16_t len)
{
	struct AT45_flash_dev * flash_dev = (struct AT45_flash_dev *)flash_id;
...
//
int32_t PIOS_Flash_AT45_PageToBuffer(uintptr_t flash_id, uint16_t addr)
{
	struct AT45_flash_dev * flash_dev = (struct AT45_flash_dev *)flash_id;
	uint8_t out[4];
...
int32_t PIOS_Flash_AT45_BufferToPage(uintptr_t flash_id, uint16_t addr)
{
	struct AT45_flash_dev * flash_dev = (struct AT45_flash_dev *)flash_id;
	uint8_t out[4];
...
int32_t PIOS_Flash_AT45_WriteBuffer (uintptr_t flash_id, uint16_t addr,  uint8_t * data, uint16_t len)
{
	struct AT45_flash_dev * flash_dev = (struct AT45_flash_dev *)flash_id;
	uint8_t out[4];
...
int32_t PIOS_Flash_AT45_ReadBuffer (uintptr_t * flash_id, unsigned char BufferNum, uint16_t IntBufferAdr,  uint8_t * data, uint16_t len)
{
	struct AT45_flash_dev * flash_dev = (struct AT45_flash_dev *)flash_id;
	uint8_t out[5];
  ...
  if (BufferNum==1)
	  out[0] = AT45_BUFFER1_READ;
  else
	  out[0] = AT45_BUFFER2_READ;
...
const struct pios_flash_driver pios_at45_flash_driver = {
	.start_transaction = PIOS_Flash_AT45_StartTransaction,
	.end_transaction   = PIOS_Flash_AT45_EndTransaction,
	.erase_chip        = PIOS_Flash_AT45_EraseChip,
	.erase_sector      = PIOS_Flash_AT45_EraseSector,
	.write_data        = PIOS_Flash_AT45_WriteData,
	.write_buffer      = PIOS_Flash_AT45_WriteBuffer,
	.buffer_to_page    = PIOS_Flash_AT45_BufferToPage,
	.page_to_buffer    = PIOS_Flash_AT45_PageToBuffer,
	.read_data         = PIOS_Flash_AT45_ReadData,
};
